...
test("background preserves browser sign-in and PKCE grants, and rejects manual key connection requests", async () => {
  ...
  globalThis.chrome = { runtime: { id: "test-extension", getURL: (path: string) => `chrome-extension://test-extension/${path}`, onInstalled: { addListener() {} }, onStartup: { addListener() {} }, onConnect: { addListener() {} }, onMessage: { addListener(fn: any) { listener = fn; } }, sendNativeMessage: async () => { throw new Error("No test host"); } },
  ...
  try {
    ...
    const untrustedUpdate = await new Promise<any>(resolve => listener({ type: "bridge:update:check" }, { id: "test-extension", url: "https://example.com" }, resolve));
