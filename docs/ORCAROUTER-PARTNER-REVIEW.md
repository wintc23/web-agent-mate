...
- 文案调整：面向用户的 README、隐私说明、商店文案和作品页只保留实际推广佣金关系，移除关于未提供奖励和额度的声明。后续浏览器操作遵循 `AGENTS.md` 中的自有工具优先规则；当前外部 Codex 会话尚未接入 WebAgentMate 浏览器工具，需从插件内发起网页任务或接入相应工具接口。下面的 Playwriter 记录属于规则调整前的检查历史。
